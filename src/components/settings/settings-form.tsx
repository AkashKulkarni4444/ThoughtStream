...
const SettingsForm = () => {
  ...
  const { user, subscription, setUser } = useSupabaseUser();
  ...
  const onChangeProfilePicture = async (
    ...
    if (!error) {
      ...
      setUser((prevUser) => {
        return { ...prevUser!, avatarUrl: data.path };
      });
      ...
      <>
        ...
        <div className="flex items-center">
          ...
          <div className="flex flex-col ml-6">
            ...
            <Input name="profilePicture" type="file" accept="image/*" placeholder="Profile Picture" disabled={uploadingProfilePic} onChange={onChangeProfilePicture} />
          </div>
        </div>
        <LogoutButton>
          <div className="flex items-center">
        ...
        {subscription?.status === "active" ? (
          <div>
            <Button type="button" size="sm" variant={"secondary"} disabled={loadingPortal} className="text-sm" onClick={redirectToCustomerPortal} >
              Manage Subscription
            </Button>
          </div>
        ) : (
